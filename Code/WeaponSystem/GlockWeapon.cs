...
public class GlockWeapon : BaseBulletWeapon
{
	...
	private void ShootBullet( bool secondary, float fireRate )
	{
		...
		var forward = Owner.Controller.EyeTransform.Rotation.Forward.WithAimCone( 0.1f + aimConeAmount * 3f, 0.1f + aimConeAmount * 3f );
		var bulletRadius = 1;

		var tr = Scene.Trace.Ray( Owner.Controller.EyeTransform.ForwardRay with { Forward = forward }, 4096 )
		...
		if ( !Owner.Controller.ThirdPerson && !Owner.IsProxy)
		{
